(()=>{
  ...
  style.textContent=`
    .mock-sleeper-ui{--mock-line:#ffffff24;--mock-panel:#07182a}
    ...
    .mock-sleeper-ui #draftBoard{display:grid!important;grid-template-columns:repeat(var(--mock-teams,10),minmax(108px,1fr))!important;gap:2px!important;overflow:auto!important;background:#04101e;border:1px solid var(--mock-line);border-radius:10px 10px 0 0;padding:2px;scrollbar-width:thin}
    .mock-sleeper-ui #draftBoard .sleeper-team-head{min-height:34px;display:flex;align-items:center;justify-content:center;padding:4px 2px;background:#0b2239;color:#9eb1c6;border-bottom:1px solid var(--mock-line);font-size:.68rem;font-weight:900;text-align:center;position:sticky;top:0;z-index:3}
    ...
    .mock-sleeper-ui .sleeper-tabs{display:flex;height:46px;background:#061522;border:1px solid var(--mock-line);border-top:0}
    .mock-sleeper-ui .sleeper-tab{flex:1;border:0;background:transparent;color:#6f8298;font-size:.72rem;font-weight:950;letter-spacing:.08em;text-transform:uppercase;position:relative}
    ...
    .mock-sleeper-ui .player-pool{position:static!important;order:initial!important;width:100%!important;max-height:none!important;border-radius:0 0 12px 12px;border-top:0;background:#07182a}
    ...
    .mock-sleeper-ui .sleeper-pool-labels span:nth-child(n+3){text-align:center}
    ...
    @media(max-width:650px){
      ...
      .phone-mode .mock-sleeper-ui #draftBoard{grid-template-columns:repeat(var(--mock-teams,10),82px)!important;max-height:255px;overflow:auto!important}
      ...
      .phone-mode .mock-sleeper-ui .pool-player{grid-template-columns:58px 32px minmax(0,1fr) 40px 40px!important;padding:8px 7px!important;gap:5px!important}
      .phone-mode .mock-sleeper-ui .pool-player .draft-btn{width:54px!important;font-size:.6rem!important}
      .phone-mode .mock-sleeper-ui .sleeper-pool-labels{grid-template-columns:58px minmax(0,1fr) 40px 40px;padding-left:7px;padding-right:7px}
  ...
  let tabs=section.querySelector('.sleeper-tabs');
  if(!tabs){
  ...
  tabs.addEventListener('click',e=>{
    const b=e.target.closest('[data-sleeper-tab]');
    if(b)setTab(b.dataset.sleeperTab);
  });
  setTab('players');

  const head=pool.querySelector('.pool-head');
  if(head&&!head.querySelector('.sleeper-pool-labels')){
  ...
  function teamCount(){return Number(document.getElementById('mockTeams')?.value)||10}
  function mySlot(){return Number(document.getElementById('mockSlot')?.value)||1}

  function ensureTeamHeaders(){
    const teams=teamCount();
    draftBoard.style.setProperty('--mock-teams',teams);
    if(!draftBoard.querySelector('.drafted'))return;
    const existing=[...draftBoard.querySelectorAll('.sleeper-team-head')];
    if(existing.length===teams)return;
    existing.forEach(x=>x.remove());
    let html='';
    for(let i=1;i<=teams;i++)html+=`<div class="sleeper-team-head${i===mySlot()?' mine':''}">Team ${i}</div>`;
  ...
  function decoratePool(){
    document.querySelectorAll('#poolList .pool-player').forEach(row=>{
      ...
      const nextAdp=String(p?.additionalStats?.espnAdp||'—');
      const nextPpg=String(getPpg(p));
      if(adp.textContent!==nextAdp)adp.textContent=nextAdp;
      if(ppg.textContent!==nextPpg)ppg.textContent=nextPpg;
    });
  }
  ...
  section.addEventListener('click',sync);
